...
export class ConsentRequestsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async searchConsentRequests(searchConsentRequestsDTO: SearchConsentRequestsDTO, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'searchConsentRequestsDTO' is not null or undefined
        if (searchConsentRequestsDTO === null || searchConsentRequestsDTO === undefined) {
            throw new RequiredError('Required parameter searchConsentRequestsDTO was null or undefined when calling searchConsentRequests.');
        }


        // Path Params
        const localVarPath = '/consent_requests/search';
        ...
        requestContext.setHeaderParam("Content-Type", contentType);
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(searchConsentRequestsDTO, "SearchConsentRequestsDTO", ""),
            contentType
        );
        requestContext.setBody(serializedBody);
...
export class ConsentRequestsApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to searchConsentRequests
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async searchConsentRequests(response: ResponseContext): Promise<PaginationResultDTOConsentRequestSearchResultDTO > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: PaginationResultDTOConsentRequestSearchResultDTO = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "PaginationResultDTOConsentRequestSearchResultDTO", ""
            ) as PaginationResultDTOConsentRequestSearchResultDTO;
            return body;
        }
        if (isCodeInRange("401", response.httpStatusCode)) {
            const body: ApiErrorDTO = ObjectSerializer.deserialize(
